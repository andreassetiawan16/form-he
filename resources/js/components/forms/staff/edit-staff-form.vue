<template>
  <div class="container-fluid">
    ...
    <button class="btn btn-info" @click="updateStaff">Save</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    async updateStaff () {
      ...
      let response = await axios({
        method: 'POST',
        url: process.env.MIX_BASE_URL + '/staff/' + this.data.id + '/update',
        data: payload
      })
  ...
  created () {
    this.data = Object.assign({}, JSON.parse(this.datastaff))
    this.data.tanggal_lahir = this.data.tanggal_lahir.substring(0, 10)
  }
}
</script>